...
**Concept considerations:** When generating concepts for Packaging/Employee, think about what ACTIONS during packing can demonstrate the product's benefits. The concept should describe what the hands do, what the voiceover says, and how the two work together to prove the claims.`,
  ...
  'AI Podcast': `**AI Podcast Ads — AI-GENERATED VIDEO**

**What it is:** Podcast-style video ads where both hosts and the entire set are generated by AI text-to-video models (Veo / Sora / Runway). No real talent, no shoot — the editor uses paste-ready avatar profiles + a shared set description to render consistent hosts across multiple clips.

**What makes it unique:** Same conversational authority as a filmed podcast, but the production lives entirely in the editing path. No casting, no set, no scheduling. The brief is the production. Two AI avatars in dialogue, rendered as many clips as needed, then assembled.

**Biggest advantage:** Speed and scalability. A filmed podcast requires casting, a studio, and a shoot day. An AI podcast can be assembled the same day. Same trust dynamic (skeptic + believer dialogue), same product-discovery beat, same conversational tone — without any production overhead.

**How it's typically made:** Editor receives the brief with two detailed Avatar Profiles (appearance, wardrobe, voice, personality), a single Set Description (lighting, mic placement, framing), and a screenplay-format dialogue script. The avatar profile and set description get pasted VERBATIM into every generation prompt for that host's clips so the avatars and set stay consistent across the assembled video.

**What the output looks like:** Looks like a filmed podcast clip — same two-host framing, same conversational rhythm, same product-discovery beat. The viewer typically can't distinguish AI-generated podcast from a real one when the avatar consistency and dialogue pacing are nailed. Usually 30-60 seconds for Meta.

**Script requirements:** Two detailed avatar profiles (appearance + wardrobe + voice + personality — paste-ready). One shared set description. Dialogue formatted as screenplay with explicit HOST A / HOST B speaker labels. The PRODUCT DISCOVERY MOMENT marked in the dialogue so the editor knows the inflection point.

**Concept considerations:** Identical to Fake Podcast (filmed) for the conversational arc — same skeptic/believer dynamic, same discovery beat, same earned product mention. The only difference vs. filmed: avatar consistency is the editor's challenge, not the producer's. Write avatars detailed enough that the AI renders the same face every clip.`,

  'Fake Podcast Ads': `**Fake Podcast Ads — VIDEO (FILMED)**

**What it is:** Filmed podcast-style ads with two real hosts in a podcast set. Production path — Nora casts real talent and shoots the conversation. (For the AI-generated version of this format, see "AI Podcast".)
...
**Concept considerations:** When generating concepts for Spokesperson, think about WHICH type of expert would have the most credibility for this angle. The concept should describe: who the expert is, what their credentials are, what specific claim they're uniquely qualified to make, and what environment reinforces their authority.`,
  ...
  'Full AI (Documentary, story, education, etc)': `**Full AI (Documentary / Story / Educational / etc.) — VIDEO**

**What it is:** Long-form-feeling video ads where 100% of the footage is generated by AI text-to-video models (Veo / Sora / Runway / Kling). No real talent, no live shoots, no Viasox stock clips. The creative strategist writes a brief that doubles as a generation prompt set, and a producer feeds each shot description into an AI model to render the visuals.

**What makes it unique:** It unlocks creative territory that traditional production can't reach economically — historical settings, surreal metaphor, time travel, dreamlike imagery, "no humans shown" perspectives, impossible scale, period-accurate documentary footage. The format trades the realism of live-action for total creative freedom and a "this isn't an ad, it's a film" feel. It's the highest-leverage format when the concept lives in story, history, education, or aspiration rather than testimonial.

**Biggest advantage:** Production-cost arbitrage on cinematic narrative. A 90-second AI documentary about the history of compression therapy, an emotional story about a woman rediscovering her independence, or an educational visual essay on venous return — all of which would cost $50K+ to produce traditionally — can be assembled from generation prompts. The format is also infinitely iterable: once a brief works, the same prompts can be regenerated with style variations, persona variations, or seasonal variations at near-zero marginal cost.

**How it's typically made:** A brief specifies the **Specification** (Documentary / Historical / Educational / Emotional Story / Aspirational) which sets the narrative mode, and a **Visual Style** (Story with cohesive characters / Fully Voice Over / Includes Talking To Camera / No Humans Shown / Historical Visuals and Claims) which sets the visual execution language. Each row of the script body becomes a generation prompt: subject, action, environment, mood, camera language, lighting. The producer renders shots one by one, then assembles in edit with voiceover and music.

**What the output looks like:** Depends on the Specification + Visual Style. A "Documentary + No Humans Shown" ad might look like a Werner Herzog short — extreme close-ups of weathered hands, feet on cold floors, archival textures, with a voiceover that earns attention by giving knowledge first. An "Emotional Story + Story with cohesive characters" ad might look like a 60-second short film — one woman, one journey, one quiet moment of dignity restored. The viewer should momentarily forget this is an ad. The product appears briefly at the end as the resolution, never the star.

**Script requirements:** Every shot description must be feasible for current AI text-to-video models. Avoid: tight branded product shots, multi-person dialogue requiring lip-sync, complex hand-product manipulation, real-world brand-accurate locations. When a character recurs across shots, describe her the same way every time so the model maintains identity consistency (approximate age, hair color/style, build, clothing palette). Voiceover dominates. Music direction matters. The Specification + Visual Style must FUNDAMENTALLY shape pacing, tone, and structure — a Documentary + Fully Voice Over ad and an Aspirational + Talking To Camera ad should look completely different.

**Concept considerations:** When generating concepts for Full AI, exploit what AI uniquely does: impossible scale, time travel, surreal transitions, visual metaphor, dreamlike imagery, historical worlds, anatomical/scientific visualizations, perspectives that no camera could capture. Do NOT describe concepts that would be better shot live — those belong in AGC or UGC. Lean into story, history, education, or aspiration. The product is the resolution; the story is the ad. Protagonist (when shown) is still a woman 50+ per the core audience mandate.`,
};
...
export function buildAdTypeGuideCompact(adType: AdType): string {
  const compactGuides: Record<AdType, string> = {
    ...
    'Fake Podcast Ads': `**Fake Podcast Ads — FILMED VIDEO.** Two REAL hosts in a podcast setup having a scripted conversation designed to feel organic. One = "host/skeptic" (asks questions the viewer thinks), other = "guest/believer" (provides answers). Write as TWO-PERSON DIALOGUE with natural interruptions and reactions. The product mention must feel earned, not forced. The skeptic-to-believer arc is the story structure. Podcast mics visible, warm lighting, conversational energy. Filmed on set with Nora's production team.`,

    'AI Podcast': `**AI Podcast — AI-GENERATED VIDEO.** Same dialogue format as Fake Podcast Ads but BOTH HOSTS AND THE SET are AI-generated by Veo/Sora/Runway. No real talent, no shoot. Brief must include detailed paste-ready Avatar Profiles for each host (appearance, wardrobe, voice, personality) and a shared Set Description — these get pasted VERBATIM into every generation prompt so the avatars and set stay consistent across all clips. Two-person dialogue with explicit HOST A / HOST B speaker labels, mark the PRODUCT DISCOVERY MOMENT in the dialogue. Editing path — no production needed.`,
    ...
    'Full AI (Documentary, story, education, etc)': `**Full AI (Documentary / Story / Educational / etc.) — VIDEO.** 100% AI-generated footage (Veo / Sora / Runway / Kling). No real talent, no live shoots, no Viasox stock clips. Each script row is a generation prompt: subject, action, environment, mood, camera language, lighting. The **Specification** sets the narrative mode (Documentary / Historical / Educational / Emotional Story / Aspirational). The **Visual Style** sets the execution (Cohesive Characters / Fully VO / Talking To Camera / No Humans Shown / Historical Visuals). Voiceover dominates the audio; describe the music explicitly. Avoid scenes AI struggles with: tight product branding, multi-person dialogue requiring lip-sync, complex hand-product manipulation, real-world brand-accurate locations. If a character recurs, describe her the same way every time for identity consistency. Product is minimal and symbolic — a brief beat near the end, a logo card. The story is the ad.`,
  };

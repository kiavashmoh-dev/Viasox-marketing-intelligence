...
**EMOTIONAL TERRITORY:** Relief, independence, dignity, "finally," style confidence, "I can do this myself"`,
    ...
    'Compression': `## PRODUCT LINE: COMPRESSION — Graduated Compression Socks (12-15 mmHg)

**PRODUCT IDENTITY:** Graduated compression at 12-15 mmHg — the "sweet spot" where compression actually works AND can be worn all day. Performance + medical with comfort and style.
...
**PRODUCT IDENTITY:** Uniform compression around the ankle and arch (NOT graduated — uniform pressure on the specific zones that need support) in a discreet ankle-length format.
...
- BEAT 4 CATEGORY REVEAL: Reveal the CATEGORY of sock that's different. NOT Viasox yet — and reveal it with CONVICTION and a concrete attribute: "There's a sock with no elastic band at the top — it stretches to 30 inches and never leaves a mark." Never a timid existence claim ("socks built differently exist," "most people don't know they're out there") — those placeholder reveals are banned.
- BEAT 5 PRODUCT REVEAL + SOFT CTA: Viasox is finally named — PROPERLY: spoken in the VO with the product line, one concrete attribute repeated, and one proof anchor (demo moment / testimonial line / "107K reviews"). Soft but confident, curiosity-driving CTA only. NEVER "Buy now," "Shop," or offers/prices. Use: "See what 107K people found," "Learn more," "Discover it," "See if it's for you." Goal is moving them from Unaware → Problem-Aware, not from scroll → checkout.
Best for: SPECIFICALLY Unaware audiences. TOF only. Gradualization maps most natively onto the Unaware release gates — but it is ONE valid engine, not the mandatory one. Any framework may drive an Unaware script (Confession Arc, Skeptic's Journey, Contrast/Split, Discovery Narrative, Day-in-the-Life…) as long as the Unaware INFORMATION-RELEASE RULES hold: elimination rules in the opening, understanding earned before category, category before brand, product last. The framework is the plot; the awareness level is the censor deciding what may be said when.`,
};
...
export function buildScriptPrompt(
  ...
  /**
   * @deprecated — retained only for caller compatibility. Each ad type now
   * has its own dedicated output template via `buildBriefTemplateOutputFormat`
   * (briefTemplates.ts). The autopilot used to set this true to force every
   * brief into the Ecom shape; that's no longer needed and the parameter
   * is ignored.
   */
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  _forceEcomTemplate?: boolean,
  ...
  /**
   * Each ad type now has its own dedicated output template via
   * `buildBriefTemplateOutputFormat` — see briefTemplates.ts. The legacy
   * useEcomTemplate / templateAdType / nonEcomOverrideBlock machinery
   * was removed because it forced an Ecom shape onto every non-AGC,
   * non-Static brief, which conflicted with the new per-ad-type templates
   * and caused empty downloads.
   */
...
${params.adType === 'Static' ? `## SCRIPT OUTPUT FORMAT — STATIC
...
- **10-Second Test:** Write out the exact words the viewer hears in the first ~10 seconds (hook + first body line). Verify they contain at least TWO concrete, filmable details AND that the target viewer would recognize "this is about me" — while the wrong viewer can tell it isn't for them.${params.awarenessLevel === 'Unaware' ? " For Unaware: the details are SCENES and BEHAVIORS — product, category, and symptom labels stay banned this early. Concrete AND compliant is the entire craft; vague scenes fail exactly like vague claims." : ''}${params.awarenessLevel === 'Product Aware' || params.awarenessLevel === 'Most Aware' ? ` For ${params.awarenessLevel}: "Viasox" must land within the first ~3 seconds of these words${params.awarenessLevel === 'Most Aware' ? ', together with the offer' : ''} — quote exactly where.` : ''}
- **Product Conviction:** Name the 2+ concrete product attributes (from the PRODUCT CONVICTION bank) the product beat will sell, the exact beat where each lands, AND the proof moment (demonstration / testimonial line / specific review-data anchor) and where it lands. Run the SWAP TEST on your plan: if the product beats would survive a swap to a competitor's sock, replan them. For Unaware: this all lives in Beats 3-5 — concreteness there does not loosen the elimination rules earlier.
...
**SELF-CHECK BEFORE PROCEEDING:** After outlining, verify: (1) talking point appears in ≥3 beats, (2) no two hooks share an archetype, (3) total word estimate is within ${durationTarget.sweetSpot}, (4) ${params.awarenessLevel === 'Unaware' ? 'Schwartz\'s Three Elimination Rules are honored in Beats 1-2' : 'the awareness level\'s structural requirements are met in the beat map'}, (5) every visual is producible for ${params.adType}, (6) the first ~10 seconds pass the 10-Second Test with concrete details quoted, (7) every beat carries a ${params.framework} stage label, (8) the product beat names ≥2 concrete attributes from the bank, contains zero banned placeholder phrases, includes a named proof moment, and passes the SWAP TEST. If any check fails, fix the outline first.
...
`;
  ...
  const memorySection = memoryBriefing
    ? `\n\n## CREATIVE INTELLIGENCE — STYLE & VOICE AWARENESS\n\nThe following briefing summarizes the creative history of this system. It is a REFERENCE, not a blacklist. Use "Proven Strengths" to understand what tonal and structural choices have worked. Use "Overused Patterns" to identify signals that may be losing impact — if a pattern has been used heavily, the question isn't "ban it" but "does it still serve THIS brief's parameters?"\n\n**RELEVANCE RULE:** Write the script that best serves this specific brief's talking point, inspiration, duration, and concept — even if it shares hook style or narrative structure with a recent brief. Repetition of a technique is fine when it's the right technique. The failure mode is writing a generic script that could work for any talking point — not writing one that shares DNA with a past script but serves THIS brief well.\n\n${memoryBriefing}`
    : '';

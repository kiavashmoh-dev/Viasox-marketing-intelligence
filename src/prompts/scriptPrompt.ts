...
- Time allocation for ${params.duration}: ${params.duration === '15s' ? '0:00-0:10 identification/story → 0:10-0:13 awareness shift → 0:13-0:15 curiosity CTA' : params.duration === '30s' ? '0:00-0:15 identification/story → 0:15-0:22 awareness shift → 0:22-0:27 solution hint → 0:27-0:30 soft CTA' : params.duration === '60s' ? '0:00-0:30 deep identification/story → 0:30-0:40 awareness shift → 0:40-0:52 solution reveal → 0:52-0:60 soft CTA' : '0:00-0:40 deep identification/story arc → 0:40-0:55 awareness shift moment → 0:55-1:15 solution reveal + mechanism → 1:15-1:30 soft CTA with emotional callback'}
...
- Time allocation for ${params.duration}: ${params.duration === '15s' ? '0:00-0:03 pain hook → 0:03-0:08 agitate → 0:08-0:12 solution bridge → 0:12-0:15 CTA' : params.duration === '30s' ? '0:00-0:05 pain hook → 0:05-0:15 agitate/intensify → 0:15-0:22 solution reveal → 0:22-0:27 proof → 0:27-0:30 CTA' : params.duration === '60s' ? '0:00-0:05 pain hook → 0:05-0:20 deep agitation → 0:20-0:35 solution reveal + mechanism → 0:35-0:50 proof cascade → 0:50-0:60 CTA' : '0:00-0:08 pain hook + recognition → 0:08-0:30 deep agitation + cause/mechanism → 0:30-0:50 solution reveal + new mechanism → 0:50-1:15 proof cascade + transformation story → 1:15-1:30 offer + CTA'}
...
- Time allocation for ${params.duration}: ${params.duration === '15s' ? '0:00-0:03 brand + new info hook → 0:03-0:10 deep proof → 0:10-0:15 CTA' : params.duration === '30s' ? '0:00-0:05 brand + new info hook → 0:05-0:15 deep proof/story → 0:15-0:22 intensified desire → 0:22-0:27 offer/urgency → 0:27-0:30 CTA' : params.duration === '60s' ? '0:00-0:05 brand + new info hook → 0:05-0:20 deep customer story → 0:20-0:35 proof cascade → 0:35-0:47 mechanism deep-dive → 0:47-0:55 offer → 0:55-0:60 CTA' : '0:00-0:08 brand + new info hook → 0:08-0:30 deep customer story arc → 0:30-0:50 proof cascade → 0:50-1:10 mechanism deep-dive + behind-the-scenes → 1:10-1:22 offer + risk reversal → 1:22-1:30 CTA'}
...
- Time allocation for ${params.duration}: ${params.duration === '15s' ? '0:00-0:03 product + offer → 0:03-0:10 urgency/details → 0:10-0:15 CTA' : params.duration === '30s' ? '0:00-0:05 product + offer/news → 0:05-0:15 quick proof/urgency → 0:15-0:22 details → 0:22-0:27 risk reversal → 0:27-0:30 CTA' : params.duration === '60s' ? '0:00-0:05 product + offer → 0:05-0:15 quick proof/social → 0:15-0:25 offer details → 0:25-0:35 urgency → 0:35-0:50 product showcase → 0:50-0:60 CTA' : '0:00-0:05 product + offer → 0:05-0:20 quick proof/social cascade → 0:20-0:40 offer details + comparison → 0:40-0:60 urgency + scarcity → 1:00-1:20 product showcase + risk reversal → 1:20-1:30 CTA'}

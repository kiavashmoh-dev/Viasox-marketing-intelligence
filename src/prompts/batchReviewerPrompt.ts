...
Does the brief actually FOLLOW the assigned framework's structure, or just name it? PAS must have clear Problem → Agitate → Solution arc. Before-After-Bridge must show the before state, the after state, and the bridge. The Gradualization must map to the 5 Unaware release gates. A brief where the framework is named but the body doesn't structurally match = 3/10. A brief where every beat maps cleanly to the framework and you could annotate each section = 9/10. IMPORTANT for Unaware briefs on frameworks OTHER than Gradualization: judge the framework's OWN arc plus compliance with the Unaware release ORDER (identification first, category before brand, product last) — do NOT demand the Gradualization 5-beat plot from a Confession Arc or Skeptic's Journey; the gates are the shared requirement, the storytelling engine is the framework's. Two Unaware briefs on different frameworks that read like the same ad with different labels = FLAG both for framework drift. Check that the framework was well-chosen for this awareness level — PAS with the problem statement up front is wrong for Unaware.

...
| `src/factory2/` + `src/components/factory2/` | **Factory V2** (ModuleId `factory2`): interactive UGC-only brief production. Briefs are TYPED OBJECTS (`UgcBriefV2`), never markdown — the AI generates/regenerates fields; hover-to-regenerate any line with a feedback ledger that binds all later generations; framework dropdown + restructure; storyboard references matched from the Inspiration Bank's frames via vision; Media Engineered template (`templateBoilerplate.ts` + `src/prompts/ugcVoiceDna.ts`). Thinking seats try `claude-fable-5`, auto-fall back to Opus 4.8. Shares V1's intake (screenshotParser/ManualTaskBuilder/asanaMapper) and knowledge modules — NOT its pipeline. |
